<?php
/**
 * functions.php - Versión 2.4 - Unificada y Definitiva
 */
...
function ghd_enqueue_assets() {
    ...
    wp_enqueue_style('ghd-style', get_stylesheet_uri(), ['parent-style'], '2.4');
    wp_enqueue_script('ghd-app', get_stylesheet_directory_uri() . '/js/app.js', [], '2.4', true);
...
// --- 3. FUNCIONES DE AYUDA (UNIFICADAS Y DEFINITIVAS) ---
...
add_action('wp_ajax_ghd_update_task_status', function() {
    ...
    if ($value === 'Completado') {
        ...
        if (get_field('estado_tapiceria', $id) == 'Completado' && get_field('estado_embalaje', $id) == 'Completado' && get_field('estado_logistica', $id) == 'No Asignado') {
            update_field('estado_logistica', 'Pendiente', $id);
            update_field('estado_pedido', 'Listo para Entrega', $id);
            wp_insert_post(['post_title' => 'Fase 2 completa -> A Logística', 'post_type' => 'ghd_historial', 'meta_input' => ['_orden_produccion_id' => $id]]);
        }
    }
